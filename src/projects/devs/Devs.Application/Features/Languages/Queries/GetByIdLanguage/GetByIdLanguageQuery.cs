...
namespace Devs.Application.Features.Languages.Queries.GetByIdLanguage
{
    public class GetByIdLanguageQuery : IRequest<LanguageGetByIdDto>
    {
        ...
        public class GetByIdLanguageQueryHandler : IRequestHandler<GetByIdLanguageQuery, LanguageGetByIdDto>
        {
            ...
            private readonly LanguageBusinessRules _languageBusinessRule;

            public GetByIdLanguageQueryHandler(ILanguageRepository languageRepository, IMapper mapper, LanguageBusinessRules languageBusinessRule)
            {
                _languageRepository = languageRepository;
                _mapper = mapper;
                _languageBusinessRule = languageBusinessRule;
            }

            public async Task<LanguageGetByIdDto> Handle(GetByIdLanguageQuery request, CancellationToken cancellationToken)
            {
                var language = await _languageRepository.GetAsync(b => b.Id == request.Id);
                _languageBusinessRule.LanguageShouldExistWhenRequested(language);
                var languageGetByIdDto = _mapper.Map<LanguageGetByIdDto>(language);
